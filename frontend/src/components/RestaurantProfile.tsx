        ...
        <div className="p-5 space-y-4">
            {
                isSeller && <div className="flex items-start justify-between">
                    <div>
                        ...
                        <div className="mt-1 flex tems-center gap-2 text-gray-500">
            ...
            {
                editMode? 
                <textarea value={description} onChange={e=>setDescription(e.target.value)} className="w-full rounded border px-3 py-2 text-sm"/>:<p className="text-sm text-gray-600">{restaurant.description ||
            ...
            <div className="flex items-center justify-between pt-3 border-t">
                ...
                <div className="flex gap-3">
                    {editMode && <button onClick={saveChanges} disabled={loading} className="flex items-center gap-1 rounded-lg bg-blue-600 px-3 py-1.5 text-sm text-white hover:bg-blue-700">
                        ...
                        {
                            isSeller && <button onClick={toggleOpenStatus} className={`rounded-lg px-4 py-1.5 text-sm font-medium text-white ${isOpen?"bg-red-600 hover:bg-red-700":"bg-green-600 hover:bg-green-700"}`}>{isOpen?"Close Restaurant":"Open Restaurant"}</button>
                        }
                </div>
            </div>
