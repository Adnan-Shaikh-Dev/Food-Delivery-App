        ...
        items.map((item)=>{
          ...
          return <div className={`relative flex gap-4 rounded-lg bg-white p-4 shadow-sm transition ${!item.isAvailable?"opacity-70":""}`}>
            <div className="relative shrink-0">
              ...
              {
                !item.isAvailable  && <span className="absolute inset-0 items-center justiify-center rounded bg-black/60 text-xs font-semibold text-white">Not Available</span>
              }
            </div>
              ...
              <div className="flex items-center justify-between">
                <p className="font-medium">₹ {item.price}</p>
                {
                  isSeller && <div className="flex gap-2">
                    <button onClick={()=> toggleAvailability(item._id)} className="rounded-lg p-2 text-gray-600 hover:bg-gray-100">{item.isAvailable?<FiEye size={18}/>:<FiEyeOff size={18}/>}</button>
...

                {
                  !isSeller && <button disabled={!item.isAvailable || isLoading}
                    onClick={()=>{}} className={`flex items-center justify-center rounded-lg p-2 ${!item.isAvailable || isLoading? "cursor-not-allowed text-gray-400":"text-red-500 hover:bg-red-50"}`}
