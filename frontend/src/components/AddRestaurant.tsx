    ...
    <div className="min-h-screen bg-gray-50 px-4 py-6">
        <div className="mx-auto max-w-lg rounded-xl bg-white p-6 shadow-sm space-y-5">
            ...
            <div className="flex items-start gap-3 rounded-lg border p-4">
                ...
                loadingLocation?"Fetching Your Location ...":location?.formattedAddress || "Unable to fetch location"}
                </div>
            </div>

            <button disabled={submitting} onClick={handleSubmit} className="w-full rounded-lg py-3 text-sm font-semibold text-white bg-[#e23744]">{submitting?"Submitting":"Add Restaurant"} </button>
        </div>
    </div>
  )
}
