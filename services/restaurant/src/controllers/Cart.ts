...
export const addToCart = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  if (cartFromDifferentRestaurant) {
    return res.status(400).json({
      message:
        "You can order from only restaurant at a time. Please clear your cart first to add items from this restaurant.",
    });
  }
  ...
  res.json({ message: "Item added to cart", cart: cartItem });
});
